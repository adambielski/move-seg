...
def _make_projector(im_res, cout, proj_type, expand=False, pretrained=True, pretrained_ckpt=None):
    ...
    if pretrained_ckpt is not None:
        ...
        for k in list(state_dict.keys()):
            ...
            del state_dict[k]
        # checkpoint['state_dict'] = {'.'.join(k.split('.')[2:]): v for k, v in checkpoint['state_dict'].items()} # Fixed for MOCO now
...
class F_RandomProj(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.pretrained, self.scratch = _make_projector(im_res=im_res, cout=self.cout, proj_type=self.proj_type, expand=self.expand, pretrained=pretrained, pretrained_ckpt=pretrained_ckpt)
